      ...
      <div className="flex min-h-dvh">
        ...
        <section className="min-w-0 flex-1">
          <header className="flex h-[82px] items-center justify-between border-b border-border/70 bg-background/90 px-5 backdrop-blur-md sm:px-8">
            ...
            <div className="flex shrink-0 items-center gap-1.5 sm:gap-4"><div className="relative hidden md:block"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Rechercher un élève..." className="h-10 w-56 rounded-lg border border-border bg-muted/35 pl-9 pr-3 text-sm outline-none transition-colors placeholder:text-muted-foreground/70 focus:border-primary focus:ring-2 focus:ring-ring/20" /></div><button className="relative flex size-9 items-center justify-center rounded-lg border border-border text-muted-foreground transition-colors hover:bg-muted sm:size-10" aria-label="Notifications"><Bell className="size-[18px]" /><span className="absolute right-2 top-2 size-1.5 rounded-full bg-primary" /></button><button onClick={toggleTheme} className="hidden size-9 items-center justify-center rounded-lg border border-border text-muted-foreground transition-colors hover:bg-muted xs:flex sm:size-10" aria-label={isDark ? 'Mode clair' : 'Mode sombre'}>{isDark ? '☀️' : '🌙'}</button><div className="hidden size-9 items-center justify-center rounded-full bg-primary text-xs font-bold text-primary-foreground sm:flex">AD</div>{!isLoggedIn ? <Button onClick={handleLogin} variant="outline" size="sm" className="shrink-0 px-2 sm:px-3">Connexion</Button> : <Button onClick={handleLogout} variant="outline" size="sm" className="shrink-0 px-2 sm:px-3">Déconnexion</Button>}</div>
          </header>
            ...
            {isLoading ? (
              ...
              <div className="space-y-6">
                {active === "Vue d'ensemble" && (
                  <>
                    <div className="mb-8 flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 font-serif text-sm italic text-primary">Bonjour Aminata,</p><h2 className="text-3xl font-semibold tracking-[-0.04em] sm:text-4xl">Voici l'état du lycée.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted-foreground">Une vue claire des effectifs, des inscriptions et de la vie scolaire pour décider plus vite.</p></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><button className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-border bg-card px-3 text-sm font-medium shadow-sm transition-colors hover:bg-muted sm:w-auto"><CalendarDays className="size-4 text-muted-foreground" />2025 — 2026<ChevronDown className="size-4 text-muted-foreground" /></button><Button className="h-10 w-full gap-2 bg-primary px-4 text-primary-foreground shadow-sm transition-transform hover:scale-[1.02] active:scale-[0.98] sm:w-auto"><ArrowUpRight className="size-4" />Rapport du jour</Button></div></div>
